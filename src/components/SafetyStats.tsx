...
import { AlertTriangle, Users, Activity, TrendingUp, ChevronDown, ChevronUp } from 'lucide-react';
      ...
      <div className="container px-4 sm:px-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {stats.map((stat, index) => (
            <Card key={index} className={`hover-card bg-black/40 backdrop-blur-md border border-white/10 overflow-hidden ${stat.hasGraph ? 'col-span-1 lg:col-span-2' : ''}`}>
              ...
              <CardContent className="pt-6 flex flex-col items-center text-center relative z-10">
                ...
                {stat.hasGraph && (
                  ...
                  >
                    <CollapsibleTrigger className="flex items-center justify-center gap-2 w-full mt-4 p-2 bg-primary/20 rounded-md text-primary hover:bg-primary/30 transition-colors">
                      {isGraphOpen ? 'Hide Trend' : 'Show Trend'} 
                      {isGraphOpen ? <ChevronUp className="h-4 w-4" /> : <ChevronDown className="h-4 w-4" />}
                    </CollapsibleTrigger>
                    <CollapsibleContent className="mt-4 w-full">
                      <div className="mt-3 py-3 bg-black/60 rounded-lg w-full h-64">
